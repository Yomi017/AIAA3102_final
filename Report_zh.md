...
- 架构概览：整体参照“多模态 ReAct Agent + 工具集 + 安全层 + 多端入口”的思路搭建。图 `/images/structure_zh.png` 展示了典型链路：用户输入在 CLI 或 Web 前端产生，经 FastAPI 或本地主进程转发给 `Agent`，由 `ToolsManager` 统一调度各类工具，并可选择性读取 `rag/wiki_vector_db`；安全模块在输入侧执行拦截，日志模块负责追踪每次 Thought/Action。  
- 核心模块角色细化：  
