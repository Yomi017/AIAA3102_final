...
- 课程背景与项目目标：面向《Python programing for AI》课程的最终项目，我们需要证明自己能够在有限资源内落地一套“可运行、可评测、可防护”的多模态智能体系统。本工作以 Qwen3/Qwen3-VL 系列模型为核心，构建一个既能在终端 CLI 运行、又能通过 FastAPI + Web UI 对外服务的 ReAct Agent，并在工具调用、RAG、ALFWorld、Web 搜索及安全评估等多个维度完成可复现实验。  
- 问题空间与挑战：  
...
2. **RAG 结果汇总**  
   - 数据来源：`benchmark_results/RAG` 与 `RAG_baseline`（多个 session 已聚合），由 `analysis/analyze_rag.py` 生成 `rag_analysis.md`。  
   - 关键指标：RAG / Baseline 各 20 条样本，平均分 **0.16 vs 0.07**（+0.09），中位数 **0.12 vs 0.06**，平均响应时长 **21.6s vs 10.9s**。  
   - 按类型：`RAG+calc` 0.20 vs 0.08（+0.12）；`RAG test` 0.12 vs 0.05（+0.07）。  
   - 结论：检索与计算结合明显提升得分，但引入约 2× 延迟；baseline 低分样例集中在未检索或计算链缺失。  

  ![RAG 平均分](benchmark_results/charts/rag_average_scores.png)
  ![RAG 类型均分](benchmark_results/charts/rag_case_type_scores.png)
  ![RAG 分布](benchmark_results/charts/rag_score_distribution.png)

3. **Web Search 结果汇总**  
   - 数据来源：`benchmark_results/web` 与 `web_baseline`，报告 `web_analysis.md`。  
   - 关键指标：平均分 **0.48 vs 0.32**（+0.16），平均用时 **28.8s vs 14.0s**。  
   - 观察：启用实时搜索获得更高逻辑/事实分，但吞吐下降；baseline 更快但常因未完成搜索流程或引用不足被扣分。常见失败原因来自搜索未执行、来源不匹配或结果整合缺失。  

  ![Web 平均分](benchmark_results/charts/web_average_scores.png)
  ![Web 延迟](benchmark_results/charts/web_elapsed.png)

4. **安全模块结果汇总**  
  - 本轮在扩展测试集中，PromptGuard 对 10 例攻击检出 8 例（漏报 20%），对 6 例安全提示 0 误报，说明启发式规则对显式注入有效，但对编码/隐晦表达存在漏检。  
  - 漏报原因：规则基于正则启发式，对变形/编码（如 Base64 包装敏感词）鲁棒性不足；未来可叠加二次 LLM 判别器或采用语义匹配以降低漏报。  
...
2. **有/无检索（RAG Ablation）**  
   ...
   - 对比结果：平均分 **0.16 (RAG) vs 0.07 (baseline)**，中位数 **0.12 vs 0.06**；`RAG+calc` 子集 +0.12，`RAG test` 子集 +0.07；代价是平均延迟增加 **~11s**。  
   - 分析：检索上下文对长尾/计算问答贡献显著，但需优化生成长度与检索缓存来降低时延。低分样例集中在 baseline 未检索或计算链中断。  
...
3. **有/无搜索增强（Web Search Ablation）**  
   ...
   - 对比结果（web vs web_baseline）：平均分 **0.48 vs 0.32**（+0.16），但延迟 **28.8s vs 14.0s**。  
   - 分析：实时搜索显著提升时效性问答质量但牺牲吞吐；多数低分源于未正确完成搜索流程或引用不足，后续可通过并行检索与结果摘要减时。  
...
- 本项目落地了一个多模态 ReAct Agent，全链路打通 CLI / FastAPI / Web UI，并在 ALFWorld、RAG、Web 三条基准上给出可复现实验与对比。  
- 关键发现：  
  - 在环境交互上，ReAct 相比基线成功率 +37 pct、动作准确率 +20 pct，证明 Thought→Action→Observation 回路能显著减少无效动作。  
  - 在检索问答上，RAG 提升平均分 0.09，但延迟约 2×；`RAG+calc` 子集收益更大，提示需要对计算型问题优先引入检索。  
  - 在时效搜索上，实时搜索提升 0.16 平均分，代价是时延翻倍，需要并行检索与结果摘要来兼顾吞吐。  
  - 安全侧 PromptGuard 在扩展集上检出率 80%、误报率 0%，对显式/直白注入有效，但对编码/隐晦攻击仍有漏报，需引入更强的语义防护。  
- 展望：  
  - 工程侧：增加检索缓存、结果摘要和并行工具调用，降低 Web/RAG 时延；在 ALFWorld 加入轨迹截断和自洽检查，缓解 decoder 截断。  
  - 模型侧：尝试多模型裁决（judge LLM）与语义安全分类器，减少 PromptGuard 漏报；对计算/规划任务探索多样采样 + 投票。  
  - 评测侧：补充安全基准（注入/数据外泄/工具滥用）和更大规模的 Web/RAG 集，形成持续可复现的回归测试套件。  